...
import { PassengerSelectPopover } from './PassengerSelect'
...
export function TrainSearchForm({ onSearch, className = '' }: TrainSearchFormProps) {
  ...
  const [highSpeedOnly, setHighSpeedOnly] = useState(true)
  const [exploreHotel, setExploreHotel] = useState(true)
    ...
    >
      <div className={`w-full overflow-hidden rounded-[20px] bg-white shadow-lg ${className}`}>
        ...
        <div className="px-4 py-4 sm:px-6 sm:py-5">
            ...
            <div className="col-span-1 xl:flex-[1.1] w-full">
              <div className="flex justify-between items-center mb-2 px-1">
                ...
                <Checkbox
                  ...
                  className="text-text-main font-semibold text-[14px] sm:!text-[15px]"
                >
                  Khứ hồi
                </Checkbox>
              </div>
          ...
          {/* Checkbox Options (Bottom Row) */}
          <div className="mt-4 px-1 flex flex-row items-center justify-between w-full">
            <Checkbox
              checked={highSpeedOnly}
              onChange={(e) => setHighSpeedOnly(e.target.checked)}
              className="text-text-main text-[14px] sm:!text-[15px] font-semibold"
            >
              Chỉ tàu cao tốc
            </Checkbox>

            <div className="flex items-center gap-2">
              <span className="text-text-main text-[14px] sm:!text-[15px] font-semibold">Khám phá khách sạn</span>
              <Checkbox
                checked={exploreHotel}
                onChange={(e) => setExploreHotel(e.target.checked)}
              />
            </div>
          </div>
