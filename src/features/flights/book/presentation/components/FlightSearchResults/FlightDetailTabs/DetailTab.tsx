...
function AmenityItem({
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className={`leading-[1.4] text-[13px] md:text-[14px] ${textClassName}`}>
        {children}
      </div>
    </div>
  )
...
function LocationDisplay({ title, subtitle, terminal }: { title: string; subtitle: string; terminal?: string }) {
  return (
    <div>
      <div className="text-[15px] md:text-[16px] font-semibold text-text-main leading-tight">{title}</div>
      <div className="text-[13px] md:text-[14px] font-semibold text-text-secondary mt-0.5">{subtitle}</div>
      {terminal && <div className="text-[13px] md:text-[14px] font-semibold text-text-secondary mt-0.5">{terminal}</div>}
    </div>
  )
}
      ...
      <div className="bg-surface-hover border-x border-border-main py-4 md:py-6 px-3 md:px-6">
        <div className="grid grid-cols-[70px_24px_1fr] md:grid-cols-[90px_40px_1fr]">
          ...
          <div className="flex items-center justify-center">
            <div className="text-[13px] md:text-[14px] font-semibold text-text-secondary">{flight.duration?.replace(/\s+/g, '') || '2h15m'}</div>
          </div>
          ...
          <div className="pb-10">
            <div className="flex items-center gap-2">
              <span className="text-[15px] md:text-[16px] font-semibold text-text-main">{flight.airline}</span>
              <AirlineLogo airline={flight.airline} logoUrl={flight.logoUrl} className="w-[24px] h-[24px] object-contain" />
            </div>
            <div className="text-[15px] md:text-[16px] font-semibold text-text-main mb-5">
