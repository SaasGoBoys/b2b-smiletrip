...
Command **không** bắt buộc có `requiredPermission` (ví dụ `LoginCommand` — chưa đăng nhập).
...
- `useAuth`: `commandBus.send(new LoginCommand(email, password))`.
- `useUsers`: `queryBus.query(new GetUsersQuery(...))`, `commandBus.send(new CreateUserCommand(...))`.
- Dùng generic TypeScript khi cần: `commandBus.send<LoginCommand, LoginCommandResult>(...)`.
...
*Tài liệu đi kèm: `01-cau-truc-du-an.md` (cấu trúc thư mục).*
